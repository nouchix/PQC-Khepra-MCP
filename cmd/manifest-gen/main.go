...
func main() {
	...
	tools := []ToolSpec{
		// ── ACP: Agent Control Plane ──────────────────────────────────────────
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("acp_status"),
		...
		},
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("acp_issue"),
		...
		},
		{
			...
			RiskClass: "destructive", Scope: "acp:write", Destructive: true,
			SchemaVersion:       "2.0.0", SchemaHash: hash("acp_revoke"),
			AllowedBackend: "in-process", TimeoutMs: 10000,
			ArgsSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
		...
		// ── ERT: Enterprise Risk & Threat (Docker sandbox) ────────────────────
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("ert_scan"),
		...
		// ── ERT Packages A–D (in-process, air-gap safe) ──────────────────────
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("ert_readiness"),
		...
		},
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("ert_architect"),
		...
		},
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("ert_crypto"),
		...
		},
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("ert_godfather"),
			AllowedBackend: "in-process", TimeoutMs: 300000,
			ArgsSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
		...
		// ── DAG Attestation ───────────────────────────────────────────────────
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("dag_attestation"),
			AllowedBackend: "in-process", TimeoutMs: 10000,
		},
		...
		// ── Godfather Report + Human-in-the-Loop Gate ─────────────────────────
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("godfather_report"),
		...
		},
		{
			...
			RiskClass:      "read_only", Scope: "compliance:report",
			SchemaVersion:       "2.0.0", SchemaHash: hash("godfather_approve"),
			AllowedBackend: "in-process", TimeoutMs: 5000,
			ArgsSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
		...
		// ── NIST Map: offline BM25 semantic search ────────────────────────────
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("nist_map"),
			AllowedBackend: "in-process", TimeoutMs: 5000,
			ArgsSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
		...
		// ── khepra_watch: continuous filesystem monitoring ─────────────────────
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("khepra_watch"),
			AllowedBackend: "in-process", TimeoutMs: 10000,
			ArgsSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
		...
		// ── SouHimBou AI: Step 01 — Discover & Classify Assets ───────────────
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("discover_assets"),
			AllowedBackend: "in-process", TimeoutMs: 60000,
		},
		...
		// ── STIG Check ───────────────────────────────────────────────────────
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("stig_check"),
			AllowedBackend: "in-process", TimeoutMs: 60000,
			ArgsSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
		...
		// ── CMMC Assessment ──────────────────────────────────────────────────
		{
			...
			RiskClass:      "read_only", Scope: "compliance:read",
			SchemaVersion:       "2.0.0", SchemaHash: hash("cmmc_assess"),
			AllowedBackend: "in-process", TimeoutMs: 60000,
			ArgsSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
		...
		// ── Agent Record (SouHimBou AI Flight Recorder) ───────────────────────
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("agent_record"),
		...
		// ── Sovereign Offline Tools ───────────────────────────────────────────
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("khepra_export_attestation"),
		...
		},
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("khepra_export_poam"),
			AllowedBackend: "in-process", TimeoutMs: 30000,
		},
		{
			...
			RiskClass:      "read_only", Scope: "compliance:read",
			SchemaVersion:       "2.0.0", SchemaHash: hash("khepra_query_stig"),
			AllowedBackend: "in-process", TimeoutMs: 5000,
			ArgsSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
		...
		},
		{
			...
			RiskClass:      "read_only", Scope: "compliance:read",
			SchemaVersion:       "2.0.0", SchemaHash: hash("khepra_get_compliance_score"),
			AllowedBackend: "in-process", TimeoutMs: 5000,
		},
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("khepra_query_threat_intel"),
			AllowedBackend: "in-process", TimeoutMs: 10000,
			ArgsSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
		...
		},
		{
			...
			RiskClass:      "read_only", Scope: "dag:read",
			SchemaVersion:       "2.0.0", SchemaHash: hash("khepra_get_dag_chain"),
			AllowedBackend: "in-process", TimeoutMs: 10000,
		},
		...
		// ── SouHimBou AI: Flight Export (Step 03) ────────────────────────────
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("flight_export"),
			AllowedBackend: "in-process", TimeoutMs: 30000,
		},
		...
		// ── OWASP Agentic Top 10 for 2026 ────────────────────────────────────
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("owasp_agent_assess"),
			AllowedBackend: "in-process", TimeoutMs: 30000,
			ArgsSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
		...
		// ── Dark Crypto Intelligence Network ──────────────────────────────────
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("dark_crypto_contribute"),
			AllowedBackend: "in-process", TimeoutMs: 30000,
		},
		...
		// ── SBOM Generation ───────────────────────────────────────────────────
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("sbom_generate"),
			AllowedBackend: "in-process", TimeoutMs: 180000,
			ArgsSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
		...
		// ── STRIDE Threat Model ───────────────────────────────────────────────
		{
			...
			SchemaVersion:       "2.0.0", SchemaHash: hash("threat_model"),
			AllowedBackend: "in-process", TimeoutMs: 60000,
			ArgsSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
	...
	sig, err := kernelports.Defaults().Signer.Sign(privKey, payloadHash[:])
	if err != nil {
