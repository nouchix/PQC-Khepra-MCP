...
func mustEnv(key string) string {
	v := os.Getenv(key)
	if v == "" {
		log.Fatalf("[webhook] required env var %q is not set", key)
	}
	return v
}
...
func main() {
	...
	log.Printf("[webhook] ASAF Stripe webhook receiver listening on %q", cfg.Addr)
	srv := &http.Server{
		Addr:         cfg.Addr,
		Handler:      mux,
...
func handleDisputeCreated(event StripeEvent) error {
	...
	log.Printf("[webhook] [URGENT] charge.dispute.created id=%q amount=%d reason=%q status=%q",
...
func sendMailImplicitSSL(addr string, auth smtp.Auth, to []string, body string) error {
	...
	for _, r := range to {
		if err := client.Rcpt(r); err != nil {
			log.Printf("[webhook] smtp rcpt %q: %v", r, err)
		}
	}
	w, err := client.Data()
	if err != nil {
