...
func main() {
	...
	log.Println("")
	log.Println("  AI Agent Scanner (public — no auth for demo funnel):")
	log.Println("    POST /api/v1/scan/agent           - Trigger 6-layer omnipotent agent scan")
	log.Println("    GET  /api/v1/scan/agent/stream    - SSE real-time scan progress")
	log.Println("    GET  /api/v1/scan/agent/:id       - Fetch completed report (authenticated)")
	log.Println("")
