...
func main() {
	...
	tools := []ToolEntry{
		...
		{Name: "acp_status", Description: "List active ACP credentials and their expiry status", RiskClass: "read_only", Scope: "acp:read", Tier: "pro", TimeoutMs: 5000},
		{Name: "acp_issue", Description: "Issue a new PQC credential via the Agent Control Plane", RiskClass: "destructive", Scope: "acp:write", Tier: "pro", TimeoutMs: 10000, Destructive: true,
		...
		{Name: "acp_revoke", Description: "Revoke an active ACP credential", RiskClass: "destructive", Scope: "acp:write", Tier: "pro", TimeoutMs: 10000, Destructive: true,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
				"credential_id": {"type": "string", "description": "Credential ID to revoke"},
			}, Required: []string{"credential_id"}}},

		// ── NHI: Non-Human Identity ────────────────────────────────────────
		{Name: "nhi_inventory", Description: "List all non-human identities (service accounts, API keys, certificates)", RiskClass: "read_only", Scope: "nhi:read", Tier: "pro", TimeoutMs: 5000},
		{Name: "nhi_orphans", Description: "Identify orphaned non-human identities with no active owner", RiskClass: "read_only", Scope: "nhi:read", Tier: "pro", TimeoutMs: 5000},
		{Name: "nhi_excessive", Description: "Identify NHIs with overly broad permissions", RiskClass: "read_only", Scope: "nhi:read", Tier: "pro", TimeoutMs: 5000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
		...
		{Name: "nhi_expired", Description: "List expired or soon-to-expire non-human identities", RiskClass: "read_only", Scope: "nhi:read", Tier: "pro", TimeoutMs: 5000},
		{Name: "nhi_revoke", Description: "Revoke a non-human identity credential", RiskClass: "destructive", Scope: "nhi:write", Tier: "pro", TimeoutMs: 10000, Destructive: true,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
				"nhi_id": {"type": "string", "description": "NHI identifier to revoke"},
			}, Required: []string{"nhi_id"}}},

		// ── ERT: Enterprise Risk & Threat Scanner ─────────────────────────
		{Name: "ert_scan", Description: "Run ERT security scan (SBOM, CVE, secrets, STIG, PQC inventory) in Docker sandbox", RiskClass: "sandboxed", Scope: "ert:scan", Tier: "enterprise", TimeoutMs: 90000, AllowedBackend: "docker",
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
		...
		{Name: "ert_readiness", Description: "Package A: NIST 800-171 Rev2 compliance assessment + live SCA risk factor. Returns alignment score (0-100), control gaps, and prioritized remediation roadmap.", RiskClass: "read_only", Scope: "ert:compliance", Tier: "enterprise", TimeoutMs: 60000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
				"project_path": {"type": "string", "description": "Path to project directory (default: current directory)"},
			}}},
		{Name: "ert_architect", Description: "Package B: Live supply chain risk — Syft SBOM + Grype CVE + CISA KEV/EPSS/MITRE enrichment. Returns findings with NIST 800-171 control mapping.", RiskClass: "read_only", Scope: "ert:supply-chain", Tier: "enterprise", TimeoutMs: 300000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
				"project_path": {"type": "string", "description": "Path to project directory (default: current directory)"},
			}}},
		{Name: "ert_crypto", Description: "Package C: PQC readiness attestation — source-level crypto primitive scan, SBOM crypto library inventory, weak primitive detection (MD5/SHA1/DES/RC4), CNSA 2.0 quantum risk context.", RiskClass: "read_only", Scope: "ert:pqc", Tier: "enterprise", TimeoutMs: 180000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
				"project_path": {"type": "string", "description": "Path to project directory (default: current directory)"},
			}}},
		{Name: "ert_godfather", Description: "Package D: EA KernelRouter causal risk attestation. Runs STIG, PQC, SBOM, Network agents in parallel, produces board-level causal chain with CVSS-band dollar impact and DAG-signed evidence node.", RiskClass: "read_only", Scope: "ert:godfather", Tier: "enterprise", TimeoutMs: 300000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
				"project_path": {"type": "string", "description": "Path to project directory (default: current directory)"},
			}}},

		// ── Compliance ─────────────────────────────────────────────────────
		{Name: "stig_check", Description: "Check a system path or configuration against STIG controls. Default: RHEL-09-STIG-V1R3. Returns CAT I/II/III findings with remediation guidance and compliance score.", RiskClass: "read_only", Scope: "stig:read", Tier: "enterprise", TimeoutMs: 60000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
		...
		{Name: "cmmc_assess", Description: "CMMC Level 1/2/3 assessment via KHEPRA compliance database (36,195 STIG→CCI→NIST→CMMC mappings). Returns satisfaction score, gaps, C3PAO readiness flag, and PQC status.", RiskClass: "read_only", Scope: "compliance:read", Tier: "enterprise", TimeoutMs: 60000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
		...
		{Name: "dag_attestation", Description: "Export the PQC-signed DAG audit trail for the current session. Returns all DAG nodes with ML-DSA-65 signatures, timestamps, and Adinkra symbol chain.", RiskClass: "read_only", Scope: "dag:read", Tier: "enterprise", TimeoutMs: 10000},
		{Name: "godfather_report", Description: "Generate an executive compliance report covering CMMC/STIG/NIST frameworks. When approval_required=true, returns a staged token — the full report is held until a human calls godfather_approve (30-min TTL).", RiskClass: "read_only", Scope: "compliance:report", Tier: "pro", TimeoutMs: 30000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
		...
		{Name: "godfather_approve", Description: "Deliver a staged executive compliance report. Requires the staged_token returned by godfather_report. Single-use — token is consumed on delivery.", RiskClass: "read_only", Scope: "compliance:report", Tier: "pro", TimeoutMs: 5000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
		...
		{Name: "khepra_export_attestation", Description: "Export PQC-signed attestation package covering all active compliance frameworks. C3PAO-ready evidence artifact — ML-DSA-65 signed, DAG-anchored.", RiskClass: "read_only", Scope: "compliance:attest", Tier: "pro", TimeoutMs: 120000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
				"project_path": {"type": "string", "description": "Path to project root (default: current directory)"},
			}}},
		{Name: "khepra_export_poam", Description: "Export a DFARS 252.204-7012 POA&M (Plan of Action & Milestones) from the current DAG state. Maps open findings to remediation timelines.", RiskClass: "read_only", Scope: "compliance:poam", Tier: "pro", TimeoutMs: 30000},

		// ── Compliance Queries ─────────────────────────────────────────────
		{Name: "khepra_query_stig", Description: "Query the embedded STIG/CCI/NIST control database (36,195 mappings). Look up STIG rules, CCI items, NIST 800-53 controls by ID or keyword.", RiskClass: "read_only", Scope: "compliance:read", Tier: "enterprise", TimeoutMs: 5000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
		...
		{Name: "khepra_get_compliance_score", Description: "Fast compliance score without full scan. Returns current CMMC/NIST/STIG satisfaction percentages from cached DAG state.", RiskClass: "read_only", Scope: "compliance:read", Tier: "enterprise", TimeoutMs: 5000},
		{Name: "khepra_query_threat_intel", Description: "Query embedded CISA KEV + CVE threat intelligence from offline database. Returns relevant CVEs, EPSS scores, MITRE ATT&CK mappings.", RiskClass: "read_only", Scope: "threat:read", Tier: "enterprise", TimeoutMs: 10000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
		...
		{Name: "khepra_get_dag_chain", Description: "Export the PQC-signed DAG chain for the current session. Returns all DAG nodes with ML-DSA-65 signatures and Adinkra symbol provenance.", RiskClass: "read_only", Scope: "dag:read", Tier: "enterprise", TimeoutMs: 10000},
		{Name: "khepra_watch", Description: "Register a filesystem path for continuous STIG-triggered scanning. Fires ert_scan on file changes.", RiskClass: "read_only", Scope: "compliance:monitor", Tier: "enterprise", TimeoutMs: 10000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
		...
		{Name: "agent_record", Description: "Record an agent action in the SouHimBou AI Flight Recorder. Records to PQC-signed DAG audit log.", RiskClass: "read_only", Scope: "audit:write", Tier: "enterprise", TimeoutMs: 15000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
		...
		{Name: "flight_export", Description: "Export a CMMC-aligned evidence packet from the SouHimBou AI flight log. Maps agent actions to NIST 800-171 / CMMC 2.0 controls.", RiskClass: "read_only", Scope: "audit:read", Tier: "enterprise", TimeoutMs: 30000},

		// ── Community Contributions ───────────────────────────────────────
		{Name: "owasp_agent_assess", Description: "Assess this MCP server deployment against OWASP Agentic Top 10 (ASI01-ASI10). Returns scored findings, active controls, gaps, and ML-DSA-65 signed evidence.", RiskClass: "read_only", Scope: "security:assess", Tier: "enterprise", TimeoutMs: 30000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
				"profile": {"type": "string", "description": "Assessment profile: quick | full (default: full)"},
			}}},
		{Name: "dark_crypto_contribute", Description: "Privacy-preserving contribution of anonymized cryptographic inventory to the Global Quantum Exposure Network. Returns aggregate quantum risk ranking.", RiskClass: "read_only", Scope: "community:contribute", Tier: "enterprise", TimeoutMs: 30000},
		{Name: "sbom_generate", Description: "Generate a CycloneDX / SPDX SBOM with PQC readiness annotations.", RiskClass: "read_only", Scope: "sbom:generate", Tier: "community", TimeoutMs: 180000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
				"project_path": {"type": "string", "description": "Path to project directory (default: current directory)"},
				"format":       {"type": "string", "description": "Output format: cyclonedx | spdx (default: cyclonedx)"},
			}}},
		{Name: "threat_model", Description: "Generate a STRIDE threat model with NIST 800-53 + MITRE ATT&CK mappings. 100% offline.", RiskClass: "read_only", Scope: "threat:model", Tier: "enterprise", TimeoutMs: 60000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
				"project_path": {"type": "string", "description": "Path to project directory (default: current directory)"},
		...
		{Name: "kasa_task", Description: "Inject a security directive into the KASA task queue, bound to an Adinkra symbol (Eban=defense, Sankofa=forensics, OwoForoAdobe=vigilance, Dwennimmen=remediation). Executed within KASA's next cognitive cycle (≤5s).", RiskClass: "read_only", Scope: "kasa:control", Tier: "enterprise", TimeoutMs: 5000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
		...
		{Name: "kasa_scan", Description: "Run KASA's port/service scanner against a target, with AI threat analysis. Maps to MITRE ATT&CK T1046. Every finding is DAG-attested with ML-DSA-65.", RiskClass: "read_only", Scope: "kasa:scan", Tier: "enterprise", TimeoutMs: 60000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
		...
		{Name: "ea_evolve", Description: "Run Evolutionary Algorithm threat evolution. Optimizes for: NIST 800-171 control coverage, PQC readiness, threat posture. Returns the fittest genome with Adinkra symbol binding.", RiskClass: "read_only", Scope: "kasa:evolve", Tier: "enterprise", TimeoutMs: 120000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
		...
		{Name: "ea_threat_score", Description: "Calculate a composite threat score for a target using the EA kernel. Returns CVSS-band score, confidence interval, and contributing factors.", RiskClass: "read_only", Scope: "kasa:read", Tier: "enterprise", TimeoutMs: 30000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
				"target": {"type": "string", "description": "Target to score (default: current directory)"},
			}}},
		{Name: "ea_risk_summary", Description: "Synthesize EA-driven risk summary: vulnerability exposure, compliance gap, PQC migration cost, blast radius. Same engine as Godfather Report.", RiskClass: "read_only", Scope: "kasa:read", Tier: "enterprise", TimeoutMs: 60000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
		...
		{Name: "threat_lookup", Description: "Look up a CVE, IP, or domain against embedded CISA KEV + MITRE ATT&CK databases. Returns CVE details, CVSS/EPSS scores, ATT&CK TTP mapping.", RiskClass: "read_only", Scope: "threat:lookup", Tier: "community", TimeoutMs: 30000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
				"query": {"type": "string", "description": "CVE ID, IP address, domain, or keyword to look up"},
			}, Required: []string{"query"}}},
		{Name: "drift_detect", Description: "Detect configuration drift between current state and a signed baseline. Uses the KHEPRA DriftEngine with Adinkra symbol binding.", RiskClass: "read_only", Scope: "compliance:drift", Tier: "enterprise", TimeoutMs: 60000},
		{Name: "ir_incident", Description: "Create a new incident in the KHEPRA IR Manager. Every incident is ML-DSA-65 signed at creation and DAG-attested.", RiskClass: "destructive", Scope: "ir:write", Tier: "enterprise", TimeoutMs: 15000, Destructive: true,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
		...
		{Name: "ir_add_ioc", Description: "Add an Indicator of Compromise to an existing incident. The IOC is ML-DSA-65 signed and DAG-recorded.", RiskClass: "destructive", Scope: "ir:write", Tier: "enterprise", TimeoutMs: 10000, Destructive: true,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
		...
		{Name: "ouroboros_stig_eye", Description: "Ouroboros STIG Eye — continuous STIG compliance monitoring status. Returns current compliance posture and drift alerts.", RiskClass: "read_only", Scope: "ouroboros:read", Tier: "enterprise", TimeoutMs: 5000},
		...
		{Name: "forensic_snapshot", Description: "Collect a DFIR-grade forensic snapshot: processes, network connections, loaded modules, environment variables. ML-DSA-65 signed, DAG-attested.", RiskClass: "read_only", Scope: "forensics:collect", Tier: "enterprise", TimeoutMs: 30000},
		{Name: "fim_baseline", Description: "Create a file integrity monitoring baseline. Monitors critical paths using SHA-256 hash baselines. Baseline is DAG-attested.", RiskClass: "read_only", Scope: "fim:write", Tier: "enterprise", TimeoutMs: 30000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
				"path": {"type": "string", "description": "Directory path to baseline"},
			}, Required: []string{"path"}}},
		{Name: "audit_dag_integrity", Description: "Full integrity audit of the KASA DAG. Verifies: node count, parent linkage, PQC metadata completeness, and signature chain.", RiskClass: "read_only", Scope: "dag:audit", Tier: "enterprise", TimeoutMs: 30000},
		...
		{Name: "fingerprint_device", Description: "Fingerprint a network device: OS detection, service banners, protocol analysis. MITRE ATT&CK T1046.", RiskClass: "read_only", Scope: "recon:read", Tier: "enterprise", TimeoutMs: 30000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
		...
		{Name: "attack_graph", Description: "Generate an attack graph from current NHI inventory. Models lateral movement paths, privilege escalation vectors, and blast radius.", RiskClass: "read_only", Scope: "threat:attack", Tier: "enterprise", TimeoutMs: 60000},
		...
		{Name: "dag_query", Description: "Query the KASA DAG for nodes matching criteria. Returns matched nodes with signatures and provenance.", RiskClass: "read_only", Scope: "dag:read", Tier: "enterprise", TimeoutMs: 10000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
		...
		{Name: "drbc_backup", Description: "Create a DRBC genesis backup of the KHEPRA project. AES-256-GCM encrypted, ML-DSA-65 signed.", RiskClass: "destructive", Scope: "drbc:write", Tier: "enterprise", TimeoutMs: 120000, Destructive: true,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
				"password": {"type": "string", "description": "Encryption password for the backup"},
			}, Required: []string{"password"}}},
		{Name: "drbc_restore", Description: "Restore the KHEPRA project from a DRBC genesis backup. Requires the same password used during backup.", RiskClass: "destructive", Scope: "drbc:write", Tier: "enterprise", TimeoutMs: 120000, Destructive: true,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
		...
		{Name: "asaf_lint", Description: "Lint an ASAF Policy Declaration Language (APDL) snippet. Validates @symbol, @framework, control block, and maps: clause presence.", RiskClass: "read_only", Scope: "compliance:lint", Tier: "enterprise", TimeoutMs: 5000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
		...
		{Name: "compliance_model_check", Description: "Verify if an LLM model is approved for the current KHEPRA deployment tier. Enforces sovereign (no commercial LLMs), hybrid, and edge tier policies from AGENTS.md.", RiskClass: "read_only", Scope: "compliance:model", Tier: "enterprise", TimeoutMs: 5000,
			ArgsSchema: &ArgsSchema{Type: "object", Properties: map[string]map[string]any{
